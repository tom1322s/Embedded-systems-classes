...
void LED_init(void)
{
    AT91C_BASE_PIOB->PIO_PER = DISPLAY_ENABLE | DISPLAY_ENABLE_RIGHT | DISPLAY_ENABLE_LEFT | DISPLAY_PINS;
    AT91C_BASE_PIOB->PIO_OER = DISPLAY_ENABLE | DISPLAY_ENABLE_RIGHT | DISPLAY_ENABLE_LEFT | DISPLAY_PINS;

    AT91C_BASE_PIOB->PIO_SODR = DISPLAY_ENABLE;

    AT91C_BASE_PIOB->PIO_SODR = DISPLAY_ENABLE_LEFT | DISPLAY_ENABLE_RIGHT;
    AT91C_BASE_PIOB->PIO_CODR = DISPLAY_PINS;
}
...
void LED_display_digit(uint8_t digit, uint8_t pos)
{
    AT91C_BASE_PIOB->PIO_SODR = DISPLAY_ENABLE_RIGHT | DISPLAY_ENABLE_LEFT;
    AT91C_BASE_PIOB->PIO_CODR = DISPLAY_PINS;
    AT91C_BASE_PIOB->PIO_SODR = LED_decode(digit);

    if (pos == RIGHT)
    {
        AT91C_BASE_PIOB->PIO_CODR = DISPLAY_ENABLE_RIGHT;
        delay_ms(10);
    }
    else
    {
        AT91C_BASE_PIOB->PIO_CODR = DISPLAY_ENABLE_LEFT;
        delay_ms(10);
    }
}
